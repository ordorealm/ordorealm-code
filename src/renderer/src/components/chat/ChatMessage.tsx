 ...
 */
export function ChatMessage({ message, showCopyButton = true, onSwitchToFileTab }: ChatMessageProps): JSX.Element | null {
  ...
  // ★ Check if this is a context summary message (from session reset) - deprecated, hidden now
  const isContextSummary = isSystem && message.id.startsWith('context-summary-');

  // ★ Hide old context-summary messages (deprecated, we now use session-inherited-divider)
  if (isContextSummary) {
    return null;
  }

  // ★ Check if this is an inherited divider message (new approach: show original messages)
  // Support both old format ('session-inherited-divider') and new format ('session-inherited-divider-xxx')
  // Also detect by content pattern for legacy messages
  const isInheritedDivider = isSystem && (
    message.id === 'session-inherited-divider' ||
    message.id.startsWith('session-inherited-divider-') ||
    (message.content && message.content.startsWith('{"rounds":'))
  );

  // ★ Parse divider content to get rounds and tokens info
  const dividerInfo = useMemo(() => {
    if (!isInheritedDivider || !message.content) return null;
    try {
      return JSON.parse(message.content);
    } catch {
      return null;
    }
  }, [isInheritedDivider, message.content]);

  // ★ Render inherited divider - a simple horizontal separator
  if (isInheritedDivider) {
    ...
    return (
      <div className="w-full py-3 px-4 my-2">
        <div className="flex items-center gap-3">
          {/* Left line */}
          <div className="flex-1 h-px bg-gradient-to-r from-transparent via-accent-indigo/30 to-accent-indigo/50" />
            ...
            {dividerInfo && (
              <>
                ...
                <span className="text-xs text-text-secondary">
                  {dividerInfo.rounds} 轮对话
                </span>
                <span className="text-xs text-text-muted">•</span>
                <span className="text-xs text-text-secondary">
                  {formatTokens(dividerInfo.tokens)} tokens
                </span>
              </>
            )}
          </div>
          ...
          {/* Right line */}
          <div className="flex-1 h-px bg-gradient-to-l from-transparent via-accent-indigo/30 to-accent-indigo/50" />
        </div>
      </div>
    );
  }
